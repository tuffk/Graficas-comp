...
void BmpImage::readBmpFile(char *filename){
	...
	if(bmpFile){
		...
		if(bitsPerPixel <= 8){
			...
			colorTable = new unsigned char[colorsUsed*4];
			fread(colorTable, 1, colorsUsed*4, bmpFile);
		}
		...
		if(compression == 0){
			...
			if(bitsPerPixel == 24){
				...
				for(i=0; i<height; ++i){
					...
					for(j=0; j<width; ++j){
						offset += 6;
						sample = imageCache[cacheOffset++]*4;
				...
				for(i=0; i<height; ++i){
					...
					for(j=0, k=0; j < width && k < paddedWidth; ++k){
						...
						for(m=0; m<samplesPerByte && j < width; m++, j++){
							offset += 6;
							samplePtr = ((sample >> bitsToShift) & filter)*4;

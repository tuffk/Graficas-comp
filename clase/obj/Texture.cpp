...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{		
		printf("Could not open texture file %s", filename);	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		printf("Could not read file header %s", filename);		// If it fails, display an error message
	...
	{
		printf("TGA file be type 2 or type 10 %s ", filename);	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{										
		printf("Could not read info header ERROR");		// Display error
	...
	{
		printf("Invalid texture information %s", filename);	// Display Error
	...
	{
		printf("Could not allocate memory for image %s", filename);	// Display Error
	...
	{
		printf("Could not read image data ERROR");		// Display Error
...
{ 
	...
	do
	{
		...
		{
			printf("Could not read RLE header %s", filename);	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					printf("Could not read image data %s", filename);		// IF we cant, display an error
				...
				{
					printf("Too many pixels read %s", filename);			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{	
				printf("Could not read from file %s", filename);			// If attempt fails.. Display error (again)
